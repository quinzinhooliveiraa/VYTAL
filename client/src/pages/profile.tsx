import { ArrowDownLeft, ArrowUpRight, Copy, History, User, Settings, LogOut, Info, Moon, Sun } from "lucide-react";
  ...
  return (
    <div className="p-6 pb-32 space-y-8">
      ...
      <div className="flex items-center gap-4 bg-card border border-border p-4 rounded-3xl shadow-sm">
        <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-primary/20 bg-muted flex items-center justify-center font-bold text-2xl text-foreground">
          AC
        </div>
        <div>
          <h2 className="font-bold text-lg">Alex Costa</h2>
          <p className="text-sm text-muted-foreground">alex.costa@email.com</p>
        </div>
      </div>
      ...
      <div className="bg-foreground text-background dark:bg-zinc-900 dark:text-white rounded-3xl p-6 relative overflow-hidden shadow-xl">
        ...
        <p className="text-background/70 dark:text-muted-foreground text-sm font-medium mb-2">Saldo Disponível para Saque</p>
        <h2 className="text-4xl font-display font-bold mb-6">R$ 132,50</h2>

        <div className="flex gap-3">
          <Button className="flex-1 h-12 rounded-xl bg-primary text-primary-foreground font-semibold hover:bg-primary/90 shadow-lg shadow-primary/20 border-none">
      ...
      {/* Fee Explanation */}
      <div className="px-4 py-3 rounded-xl bg-blue-500/10 border border-blue-500/20 text-xs text-blue-700 dark:text-blue-300 text-center flex items-center gap-2">
        <Info size={16} className="shrink-0" />
        <p>A plataforma retém <strong>10%</strong> do prêmio total para manutenção. Depósitos e saques são gratuitos.</p>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Button variant="link" className="text-xs h-auto p-0">Ver tudo</Button>
        </div>
      ...
      <div className="space-y-4 pt-4 border-t border-border">
        ...
        <Button variant="ghost" className="w-full justify-start text-destructive hover:text-destructive hover:bg-destructive/10 h-14 rounded-2xl">
